...
export function AvatarEditorModal({ user, onClose, onSaved }: { user: SafeUser; onClose: () => void; onSaved: (user: SafeUser) => void }) {
  ...
  function selectFile(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    setSource(URL.createObjectURL(file)); setError(''); setZoom(1); setRotation(0); setCrop({ x: 0, y: 0 });
  }
  ...
  return <Modal title="Profil fotoğrafını değiştir" onClose={onClose}>
    <div className="space-y-4">
      <label className="block text-xs text-text-secondary">Görsel seç<input className="mt-1.5 block w-full text-sm text-text-primary" type="file" accept="image/*" onChange={selectFile} disabled={pending} /></label>
      {source && <><div className="relative h-64 overflow-hidden rounded-xl border border-border-subtle bg-bg-elevated"><Cropper image={source} crop={crop} zoom={zoom} rotation={rotation} aspect={1} cropShape="round" showGrid={false} onCropChange={setCrop} onZoomChange={setZoom} onCropComplete={(_, pixels) => setCroppedAreaPixels(pixels)} /></div><label className="block text-xs text-text-secondary">Yakınlaştır<input className="mt-2 w-full accent-text-primary" type="range" min={1} max={3} step={0.1} value={zoom} onChange={(event) => setZoom(Number(event.target.value))} disabled={pending} /></label><div className="flex gap-2"><button className="button-secondary" type="button" onClick={() => setRotation((value) => value - 90)} disabled={pending}>↺ Döndür</button><button className="button-secondary" type="button" onClick={() => setRotation((value) => value + 90)} disabled={pending}>↻ Döndür</button></div></>}
      {error && <p className="text-xs text-accent-danger-text" role="alert">{error}</p>}
      <CooldownBadge nextAllowedAt={cooldown} />
      <div className="flex items-center justify-between gap-2"><div>{(user.avatarUrl || user.avatar) && <button className="button-secondary" type="button" onClick={() => void removeAvatar()} disabled={pending}>Avatarı kaldır</button>}</div><div className="flex gap-2"><button className="button-secondary" type="button" onClick={onClose} disabled={pending}>İptal</button><button className="h-9 rounded-lg bg-text-primary px-3.5 text-xs font-medium text-bg-surface" type="button" onClick={() => void submit()} disabled={pending || !source}>{pending ? 'Yükleniyor…' : 'Kaydet'}</button></div></div>
    </div>
  </Modal>;
}
