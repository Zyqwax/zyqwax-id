...
export default function LoginHistoryPage() {
  ...
  useEffect(() => {
    request<{ records: RecordItem[] }>("/api/admin/login-history")
      .then((result) => setRecords(result.records))
      .catch((cause) => setError(cause instanceof ApiError ? cause.message : "Login kayıtları yüklenemedi."))
  ...
  return (
    <section className="space-y-5">
      <header>
        ...
        <h1 className="mt-1 text-3xl font-semibold">Login kayıtları</h1>
      </header>
      {error && (
          ...
          <tbody>
            {records.map((record) => (
              <tr key={record.id} className="border-t border-zinc-800">
                ...
                <td className={`p-2 ${record.success ? "text-emerald-400" : "text-red-400"}`}>
                  {record.success ? "Başarılı" : "Başarısız"}
                </td>
                <td className="p-2">{record.ipAddress}</td>
                <td className="max-w-xs truncate p-2 text-zinc-400">{record.userAgent}</td>
                <td className="p-2 text-zinc-400">{date(record.createdAt)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {!records.length && <p className="p-4 text-zinc-500">Login kaydı yok.</p>}
      </div>
    </section>
  );
}
