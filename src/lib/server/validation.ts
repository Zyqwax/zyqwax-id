...
export const passwordSchema = z.string().min(8).regex(/\d/, 'şifre en az bir rakam içermeli');
export const registerSchema = z.object({ email: z.string().email().max(254), username: usernameSchema, password: passwordSchema }).strict();
...
export const OAUTH_SCOPES = ['profile', 'email', 'friends', 'blocks'] as const;
...
export function normalizeScope(scope: string): string | null {
  ...
  return requested.every((item) => OAUTH_SCOPES.includes(item as (typeof OAUTH_SCOPES)[number])) ? requested.join(' ') : null;
}
