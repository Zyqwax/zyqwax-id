...
export async function sendVerificationEmail(userId: string, email: string): Promise<void> {
  ...
  await prisma.verificationToken.create({ data: { userId, type: VerificationTokenType.EMAIL_VERIFY, tokenHash: hashToken(token), expiresAt: tokenExpiry(VerificationTokenType.EMAIL_VERIFY) } });
  const verificationLink = `${appUrl()}/verify-email?token=${encodeURIComponent(token)}`;
  let html = '';
  renderVerifyEmailTemplate({ verificationLink }, (rendered) => { html = rendered; });
...
export async function sendPasswordResetEmail(userId: string, email: string): Promise<void> {
  ...
  await prisma.verificationToken.create({ data: { userId, type: VerificationTokenType.PASSWORD_RESET, tokenHash: hashToken(token), expiresAt: tokenExpiry(VerificationTokenType.PASSWORD_RESET) } });
  const resetLink = `${appUrl()}/reset-password?token=${encodeURIComponent(token)}`;
  let html = '';
  renderResetPasswordTemplate({ resetLink }, (rendered) => { html = rendered; });
